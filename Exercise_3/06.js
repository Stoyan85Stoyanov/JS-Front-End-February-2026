...
function passwordValidator(password) {
    ...
    if (!validPasswordCharacters(password)) {
        console.log('Password must consist only of letters and digits');
    }

    if (!validPasswordDigits(password)) {
    ...
    function validPasswordDigits(password) {

        const digits = password
            .split('')
            .filter(character => Number(character) >= 0);
