...
function perfectNumber(number) {
    ...
    const properDivisorsSum = sumNumbers(properDivisors);

    console.log(properDivisorsSum === number
        ? "We have a perfect number!" : "It's not so perfect.");


    function sumNumbers(numbers) {
        const sumProperPositiveDivisors = numbers.reduce((acc, number) => acc + number, 0);

        return sumProperPositiveDivisors;
    }
}
