function manageCourses(input) {
    ...
    for (const line of input) {
        if (line.includes(': ')) {
            ...
            const studentMatch = line.match(/(.+)\[(\d+)\] with email (.+) joins (.+)/);

            if (!studentMatch) {
               continue; 
            } 

            const [, username, creditsStr, email, courseName] = studentMatch;
